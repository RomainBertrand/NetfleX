...
Functions:
init_data -- Creation of the dictionnary that links movies to all the users that have seen them and their rating, and of the dictionnary that links users to the films they have seen and their ratings
...
def init_data(with_ratings: bool = False) -> (dict, dict, dict, int):
    """Return dictionnaries that link users, movies and ratings as well as the number of views of each and all movies
    ...
    dictionnary_views_number (dict): Keys: id of movies (str), values: number of view of this movie (int)
...
def init_movies_names() -> (dict, dict):
    """Return the dictionnariy that links a movie to its id, and the reversed version of this dictionnary
...
def init_tags_movies() -> (dict, set):
    ...
    Returns:
    dictionnary_tags_movies (dict): Keys: movie_id (str), values: tags of this movie (list of string)
    list_of_all_tags (set): Set of all the tags (str) that appear in the database. No tag is: '(no genres listed)'
    """
