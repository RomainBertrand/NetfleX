            ...
            for film_form in formset:
                ...
                for elem in films_choisis:
                    if elem[0] == titre:
                        ...
                        deja_choisi = True
                if not deja_choisi and titre != None:
                    films_choisis.append([titre, note])
...
def conseil(request):#->HttpResponse
    ...
    if request.method == 'POST':
        ...
        if aime_ou_deteste.is_valid():
            ...
            tags_films_choisis = tags_pour_liste_de_films(numeros_films_possibles)
            ...
            tableau_tags = [] #[[False for tag in tags_films_choisis[num_film]] for num_film in numeros_films_possibles]
            for ind, num_film in enumerate(numeros_films_possibles):
                film_avec_tag.append([films_possibles[ind], tags_films_choisis[str(num_film)]])
                tableau_film_courant = [films_possibles[ind]] #[(films_possibles[ind], True)]
